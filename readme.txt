...
Basically, it can be used both on PC and Tablet. So, it means it can be controlled by touch or mouse/keyboard. There is no need to change control methods, which means you can use both if you want. 

Music and game difficulty can be adjust in the Setting menu.  

How to play menu briefly tells you how to play the game.

###### Tablet Mode:

- Buttons are attached to the screen, in order to control the  blocks or access to game settings. Arrows are to control the horizontal movement of the cube. X, Y, Z rotation button will change the blocks rotation in x-axis, y-axis and z-axis. 
- Accelerometer is used to accelerate the falling speed of the cube. To accelerate, you need to "throw" your tablet forward and downward (rotate your wrist in a high speed).
- Use one finger to adjust the camera view
- Use two finger to zoom in/out
- A "back to origin" button is attached to the game play screen, in order to set the camera view to default
...
- A spawner is used to generate the spawner. Spawners are generated randomly using prefabs.  A Prefab Shader script is attached to the prefabs, in order to use Phong illumination model and custom shadow map. The Group.cs script is used to set the game rules and make transforms to the blocks.
